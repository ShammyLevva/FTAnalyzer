...
namespace FTAnalyzer.Forms
{
    public partial class Census : Form
    {
        ...
        void ResetTable()
        {
            DataGridViewColumn? pos = dgCensus.Columns[nameof(IDisplayCensus.Position)];
            DataGridViewColumn? famID = dgCensus.Columns[nameof(IDisplayCensus.FamilyID)];
        ...
        void StyleRows()
        {
            try
            {
                if (dgCensus.SortedColumn is null || dgCensus.RowCount == 0)
                    return;
        ...
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0 && e.RowIndex < dgCensus.RowCount)
            {
                DataGridViewCell cell = dgCensus.Rows[e.RowIndex].Cells[e.ColumnIndex];
                CensusIndividual? ind = dgCensus.DataBoundItem(e.RowIndex) as CensusIndividual;
        ...
        void DgCensus_Sorted(object sender, EventArgs e)
        {
            if (IsHandleCreated)
                BeginInvoke(new Action(() => StyleRows()));
            else
                StyleRows();
        }
