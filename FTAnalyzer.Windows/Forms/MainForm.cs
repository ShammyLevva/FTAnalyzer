...
namespace FTAnalyzer
{
    ...
    {
        public static readonly string VERSION = "11.0.0.0-beta1";
        ...
        void SetHeightWidth()
        {
            ...
            int Width = RegistrySettings.GetIntRegistryValue("Mainform size - width", mainForm.Width);
            int Height = RegistrySettings.GetIntRegistryValue("Mainform size - height", mainForm.Height);
            int Top = RegistrySettings.GetIntRegistryValue("Mainform position - top", mainForm.Top);
            int Left = RegistrySettings.GetIntRegistryValue("Mainform position - left", mainForm.Left);
            string maxState = (WindowState == FormWindowState.Maximized).ToString();
            string maximised = RegistrySettings.GetStringRegistryValue("Mainform maximised", maxState).ToString() ?? maxState;
        ...
        public void SetDataErrorsCheckedDefaults(CheckedListBox list)
        {
            ...
            foreach (DataErrorGroup dataError in ft.DataErrorTypes)
            {
                ...
                bool itemChecked = RegistrySettings.GetBoolRegistryValue(dataError.ToString(), true);
        ...
        async void TabControl_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        bool todaysMonth = RegistrySettings.GetBoolRegistryValue("Todays Events Month", false);
                        int todaysStep = RegistrySettings.GetIntRegistryValue("Todays Events Step", 5);
        ...
        async void LoadLocationsTab()
        {
            ...
            tspbTabProgress.Visible = false;
            await Analytics.TrackAction(Analytics.MainFormAction, Analytics.LocationTabViewed);
        }
        ...
        void TabMainListSelector_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (tabMainListsSelector.SelectedTab == tabSources)
            {
                ...
                DataGridViewColumn? sourceID = dgSources.Columns[nameof(IDisplaySource.SourceID)];
                if (sourceID is not null)
                    dgSources.Sort(sourceID, ListSortDirection.Ascending);
                dgSources.Focus();
                mnuPrint.Enabled = true;
            ...
            else if (tabMainListsSelector.SelectedTab == tabOccupations)
            {
                ...
                DataGridViewColumn? occupation = dgOccupations.Columns[nameof(IDisplayOccupation.Occupation)];
                if (occupation is not null)
                    dgOccupations.Sort(occupation, ListSortDirection.Ascending);
                dgOccupations.Focus();
                mnuPrint.Enabled = true;
                tsCountLabel.Text = Messages.Count + list.Count.ToString("N0");
            ...
            else if (tabMainListsSelector.SelectedTab == tabCustomFacts)
            {
                ...
                DataGridViewColumn? customFactName = dgCustomFacts.Columns[nameof(IDisplayCustomFact.CustomFactName)];
        ...
        private void SetupFamiliesTab()
        {
            ...
            DataGridViewColumn? famID = dgFamilies.Columns[nameof(IDisplayFamily.FamilyID)];
            if (famID is not null)
                dgFamilies.Sort(famID, ListSortDirection.Ascending);
            dgFamilies.Focus();
            mnuPrint.Enabled = true;
            tsCountLabel.Text = Messages.Count + list.Count.ToString("N0");
        ...
        void SetupIndividualsTab()
        {
            ...
            DataGridViewColumn? indID = dgIndividuals.Columns[nameof(IDisplayIndividual.IndividualID)];
            if (indID is not null)
                dgIndividuals.Sort(indID, ListSortDirection.Ascending);
        ...
        void RestoreToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string directory = RegistrySettings.GetStringRegistryValue("Geocode Backup Directory", myDocuments);
        ...
        async void MainForm_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (files is not null)
            {
                ...
                {
                    if (Path.GetExtension(draggedFilename.ToLower()) == ".ged")
                    {
                        fileLoaded = true;
                        await LoadFileAsync(draggedFilename);
                        break;
                    }
                }
                if (!fileLoaded)
        ...
        async Task SetPossibleDuplicates()
        {
            ...
            tsHintsLabel.Text = string.Empty;
            duplicateData = await Task.Run(() => ft.GenerateDuplicatesList(score, ignoreUnknownTwins, progress, progressText, maxScore, cts.Token));
            cts = null;
        ...
        {
            IEnumerable<Individual> directs = ft.AllIndividuals.Filter(x => x.RelationType == RelationshipType.DIRECT || x.RelationType == RelationshipType.DESCENDANT);
                                      ...
                                      string unrecognisedFilename, string privateWarning)
        {
            try
            {
                ...
                string initialDir = RegistrySettings.GetStringRegistryValue("Report Unrecognised Census References Path", myDocuments);
        ...
        async Task SetupLooseBirthsAsync()
        {
            try
            {
                ...
                if (forenames is not null && surname is not null)
                {
                    dgLooseBirths.Sort(forenames, ListSortDirection.Ascending);
                    dgLooseBirths.Sort(surname, ListSortDirection.Ascending);
                }
                dgLooseBirths.Focus();
                mnuPrint.Enabled = true;
        ...
        async Task SetupLooseDeathsAsync()
        {
            try
            {
                ...
                if (forenames is not null && surname is not null)
                {
                    dgLooseDeaths.Sort(forenames, ListSortDirection.Ascending);
                    dgLooseDeaths.Sort(surname, ListSortDirection.Ascending);
                }
                dgLooseDeaths.Focus();
                mnuPrint.Enabled = true;
        ...
        async Task SetupLooseInfoAsync()
        {
            try
            {
                ...
                if (forenames is not null && surname is not null)
                {
                    dgLooseInfo.Sort(forenames, ListSortDirection.Ascending);
                    dgLooseInfo.Sort(surname, ListSortDirection.Ascending);
                }
                dgLooseInfo.Focus();
                mnuPrint.Enabled = true;
        ...
        public void SetFactTypeList(CheckedListBox ckbFactSelect, CheckedListBox ckbFactExclude, Predicate<ExportFact> filter)
        {
            ...
            foreach (string factType in factTypes)
            {
                if (!ckbFactSelect.Items.Contains(factType))
                {
                    ...
                    bool itemChecked = RegistrySettings.GetBoolRegistryValue($"Fact: {factType}", true);
        ...
        async void BtnShowSurnames_Click(object sender, EventArgs e)
        {
            ...
            DataGridViewColumn? surname = dgSurnames.Columns[nameof(IDisplaySurnames.Surname)];
            if (surname is not null)
                dgSurnames.Sort(surname, ListSortDirection.Ascending);
            dgSurnames.Focus();
        ...
        void MnuJSON_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string myDocuments = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
                using SaveFileDialog saveFileDialog = new();
                string initialDir = RegistrySettings.GetStringRegistryValue("JSON Export Path", myDocuments);
                saveFileDialog.InitialDirectory = initialDir ?? myDocuments;
        ...
        async void MnuGoogleMyMaps_Click(object sender, EventArgs e)
        {
            try
            {
                string myDocuments = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
                using SaveFileDialog saveFileDialog = new();
                string initialDir = RegistrySettings.GetStringRegistryValue("Google MyMaps Path", myDocuments);
                string initialFile = RegistrySettings.GetStringRegistryValue("Google MyMaps Filename", myDocuments);
                saveFileDialog.InitialDirectory = initialDir ?? myDocuments;
